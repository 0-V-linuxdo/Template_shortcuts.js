...
test("Grok metadata carries the requested release version and update log", () => {
    assert.match(manifest, /name: "\[Grok\] 快捷键跳转 \[20260819\] v1\.0\.8"/);
    assert.match(manifest, /version: "\[20260819\] v1\.0\.8"/);
    assert.match(manifest, /updateLog: "1\.0\.8: Imagine 页 New Chat/);
});
