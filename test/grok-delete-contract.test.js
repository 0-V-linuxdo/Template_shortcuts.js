...
test("Grok metadata carries the requested release version and update log", () => {
    assert.match(manifest, /name: "\[Grok\] 快捷键跳转 \[20260820\] v1\.0\.0"/);
    assert.match(manifest, /version: "\[20260820\] v1\.0\.0"/);
    assert.match(manifest, /updateLog: "1\.0\.0: 使用 grok.com 官方模型\/操作图标/);
});
