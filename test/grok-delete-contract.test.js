...
test("Grok metadata carries the requested release version and update log", () => {
    assert.match(manifest, /name: "\[Grok\] 快捷键跳转 \[20260819\] v1\.0\.6"/);
    assert.match(manifest, /version: "\[20260819\] v1\.0\.6"/);
    assert.match(manifest, /updateLog: "1\.0\.6: New Chat 改为 SPA pushState；Search\/Private 改为 selector 点击/);
});
