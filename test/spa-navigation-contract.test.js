...
test("SPA navigation skips fake popstate on Next.js history state", () => {
    ...
    assert.match(source, /if \(!nextJs\) \{\s*window\.dispatchEvent\(new PopStateEvent\("popstate"/);
    assert.doesNotMatch(
        source,
...
test("Template core version is bumped for the SPA navigation fix", () => {
    assert.match(header, /@version\s+\[20260819\] v1\.1\.2/);
