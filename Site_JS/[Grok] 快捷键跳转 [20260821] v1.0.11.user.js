...
// @name:en        [Grok] Shortcut Jump [20260821] v1.0.11
...
// @update-log     1.0.11: 将 Heavy 模型图标替换为 grok.com 官方「Team of Experts」三格连线图标。
// @update-log:en  1.0.11: Replaced the Heavy model icon with the official grok.com Team of Experts three-square connector glyph.
  ...
  (function() {
    ...
    function navigateGrokSpa(url) {
      try {
        ...
        const state = nextJs && prevState && typeof prevState === "object" && !Array.isArray(prevState) ? { ...prevState } : { url: urlObj.href };
    ...
    async function newChatAction({ engine: engine2 } = {}) {
      ...
      try {
        const urlHandler = engine2?.core?.actions?.get?.("url");
