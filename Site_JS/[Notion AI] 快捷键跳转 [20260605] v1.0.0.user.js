...
// @update-log     1.0.0: 修复 Research 模式快捷键按 Notion 当前 Settings > Mode 子菜单路径执行，增强 Mode 行 hover/click 与选中完成确认，并配合 Template core 等待异步工具快捷键与当前 hotkey 重试暂停保护。
// @update-log:en  1.0.0: Fixed the Research mode shortcut to use Notion's current Settings > Mode submenu path, hardened Mode row hover/click plus selection confirmation, and paired it with Template core async tool shortcut waiting and current-hotkey retry/pause protection.
...
(() => {
  ...
  (function() {
    ...
    function findAnyOpenModeMenuItem(target) {
      if (!target) return null;
      return findOpenModeMenuItem(target) || findOpenSettingsMenuItemByText((value) => textLooksLikeModeTargetMenuItem(value, target));
    }
    ...
    function composerShowsModeTarget(target) {
      ...
      const candidates = safeQueryAll(root, 'button, [role="button"], [aria-label], [title], div, span').filter((element) => {
