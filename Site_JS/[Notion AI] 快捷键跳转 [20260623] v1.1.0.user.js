...
(() => {
  ...
  (function() {
    ...
    function repaintSvgText(svgText, paintColor) {
      ...
      return String(svgText || "").trim().replace(/currentColor/g, color).replace(/\b(fill|stroke)=(["'])(.*?)\2/gi, (match, attr, quote, value) => {
    ...
    async function clickConversationMenuItem({ shortcut, engine: engine2 } = {}) {
      ...
      do {
        ...
        if (target) {
          if (shouldSyncDeleteTopicIcon(shortcut, spec)) {
            syncNotionDeleteTopicShortcutIconFromElement(engine2, target);
          }
    ...
    function collectModeMenuRootCandidates(target = null) {
      ...
      for (const element of textCandidates) {
        ...
        const isModeChoice = target ? textLooksLikeModeTargetMenuItem(text, target) : ["default", "ask", "plan", "research"].some((token) => normalizeNotionText(text).includes(token));
    ...
    function findOpenModeMenuRoot(modeRow = null, target = null) {
      const roots = collectModeMenuRootCandidates(target).map((root) => ({ root, score: scoreModeMenuRoot(root, modeRow), rect: getElementRect(root) })).filter((item) => item.score > 0);
    ...
    function findSettingsModeRow(root) {
      const roots = root ? [root] : safeQueryAll(document, NOTION_SETTINGS_MENU_ROOT_SELECTOR).filter((item) => item && isVisibleElement(item) && !isInsideShortcutUi(item));
      const candidates = [];
      const seen = /* @__PURE__ */ new Set();
    ...
    function countModeMenuCandidates(target, modeRow = null) {
      return collectModeMenuRootCandidates(target).map((root) => scoreModeMenuRoot(root, modeRow)).filter((score) => score > 0).length;
    }
    ...
    function formatModeActionDiagnostics(details = {}) {
      ...
      const focusInsideMenu = typeof details.focusInsideMenu === "boolean" ? details.focusInsideMenu ? "yes" : "no" : "";
      const fields = {
    ...
    function temporarilyDisableQuickInputOverlayPointerEvents() {
      const previous = safeQueryAll(document, "#notion-quick-input-overlay").filter(Boolean).map((element) => ({ element, value: element.style.pointerEvents }));
      ...
      return () => {
        for (const item of previous) {
          try {
            item.element.style.pointerEvents = item.value || "";
          } catch {
          }
        }
      };
    ...
    function focusModeRowForSubmenu(root, modeRow, diagnostics = null) {
      ...
      for (const target of focusTargets) {
        ...
        const active2 = document?.activeElement || null;
        const focusInsideMenu2 = !!active2 && (active2 === modeRow || !!modeRow?.contains?.(active2) || active2 === root || !!root?.contains?.(active2));
        if (focusInsideMenu2) break;
      }
      const active = document?.activeElement || null;
      const focusInsideMenu = !!active && (active === modeRow || !!modeRow?.contains?.(active) || active === root || !!root?.contains?.(active));
      if (diagnostics) {
    ...
    function dispatchModeRowArrowRight(root, modeRow) {
      const clickable = getClickableActionElement(modeRow, root) || modeRow;
      const active = document?.activeElement || null;
      const activeInMenu = active && (active === modeRow || !!modeRow?.contains?.(active) || active === root || !!root?.contains?.(active));
      const targets = Array.from(new Set([
    ...
    async function ensureModeMenuOpen(trigger, root, target, modeRowHint = null, diagnostics = null) {
      ...
      const modeRow = modeRowHint || findSettingsModeRow(root) || findSettingsModeRow() || findSettingsMenuItemByText(root, textLooksLikeModeMenuItem) || findOpenSettingsMenuItemByText(textLooksLikeModeMenuItem);
    ...
    function textLooksLikeAttachFileMenuItem(value) {
      const text = normalizeNotionText(value);
      return text === "add images, pdfs, or csvs" || text === "add images pdfs or csvs" || text === "add images, pdfs, csvs" || text === "attach file" || text === "upload file" || text === "上传文件" || text === "添加图片、pdf 或 csv" || text === "添加图片 pdf 或 csv" || text === "添加图片、pdf、csv";
    }
    ...
    function findAttachFileMenuItem(root) {
      if (!root) return null;
      return safeQueryAll(root, NOTION_CONTEXT_MENU_ITEM_SELECTOR).find((element) => element && isVisibleElement(element) && !isElementDisabled(element) && textLooksLikeAttachFileMenuItem(getElementText(element))) || null;
    }
    function findContextMenuRoot(triggerEl = null) {
      ...
      return safeQueryAll(document, NOTION_CONTEXT_MENU_ROOT_SELECTOR).find((element) => element && isVisibleElement(element) && (findCreateImageMenuItem(element) || findAttachFileMenuItem(element))) || null;
    }
    ...
    async function toggleImageGenerationAction({ engine: engine2 } = {}) {
      ...
      syncNotionAttachFileShortcutIconFromElement(engine2, findAttachFileMenuItem(root));
    ...
    async function openModelPickerAction({ engine: engine2 } = {}) {
      const trigger = findModelTriggerElement();
      if (!trigger) return false;
      const root = await ensureModelMenuOpen(trigger);
      if (root) syncNotionModelShortcutIconsFromMenuRoot(engine2, root);
      return !!root;
    }
...
`)) {
    ...
    function isLegacyNotionModeFallbackIconSource(source) {
      ...
      return svgText.includes("18.125 7.188") && svgText.includes("2.813 2.187") || svgText.includes("m12.166 5.625") || svgText.includes("M17.5 9.375c0 4.24") || svgText.includes("M11.25 3.75") || svgText.includes("M.625 7.188v1.875");
    }
    ...
    function syncNotionDefaultModelHotkeyOrder(shortcuts, { forceKeys = null } = {}) {
      ...
      const forcedKeySet = forceKeys instanceof Set ? forceKeys : /* @__PURE__ */ new Set();
      let changed = false;
      const modelShortcuts = shortcuts.map((shortcut) => ({ shortcut, key: getShortcutModelDefaultKey(shortcut) })).filter((item) => item.key);
      const modelShortcutSet = new Set(modelShortcuts.map((item) => item.shortcut));
      const hotkeysInUse = /* @__PURE__ */ new Map();
    ...
    function syncNotionDefaultModelShortcutOrder(shortcuts) {
      ...
      const orderedModels = NOTION_MODEL_SHORTCUT_KEYS.map((key) => modelByKey.get(key)).filter(Boolean);
      const reordered = [
    ...
    function migrateNotionManagedModelShortcuts() {
      ...
      let changed = false;
      const next = [];
      const newlyAddedKeys = /* @__PURE__ */ new Set();
    ...
    function getCssIconSource(element) {
      ...
      for (const pseudo of pseudos) {
        let style = null;
        try {
          style = window.getComputedStyle(element, pseudo || void 0);
        } catch {
          style = null;
        }
        if (!style) continue;
    ...
    function sanitizeNotionNativeSvgIcon(svg, { paintMode = "adaptive", paintColor = "currentColor" } = {}) {
      ...
      try {
        ...
        const paintTags = /* @__PURE__ */ new Set(["path", "circle", "rect", "ellipse", "line", "polyline", "polygon", "text"]);
    ...
    function extractNotionNativeIconInfoFromElement(root, { paintMode = "adaptive" } = {}) {
      ...
      for (const svg of getElementsIncludingRoot(root, "svg")) {
        ...
        if (paintMode === "original") {
          const source2 = serializeSvgIconSource(svg, { paintMode });
          if (source2) return { icon: source2, iconDark: "", iconAdaptive: false };
          continue;
        }
    ...
    function findNotionAiFaceIconSource() {
      const candidates = safeQueryAll(document, "img, [role='img'], [alt*='Notion AI' i], [aria-label*='Notion AI' i]").filter((element) => {
      ...
      }).map((element) => ({ element, source: extractNotionNativeIconSourceFromElement(element), rect: getElementRect(element) })).filter((item) => !!item.source);
      candidates.sort((a, b) => {
    ...
    function findVisibleNativeElement(selector) {
      return safeQueryAll(document, selector).find((element) => element && isVisibleElement(element) && !isInsideShortcutUi(element) && !isElementDisabled(element)) || null;
    }
