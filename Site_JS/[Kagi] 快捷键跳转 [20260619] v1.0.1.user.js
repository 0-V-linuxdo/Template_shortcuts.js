...
// @icon           data:image/svg+xml,%3Csvg%20viewBox%3D%220%200%2064%2064%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20aria-hidden%3D%22true%22%20role%3D%22img%22%20preserveAspectRatio%3D%22xMidYMid%20meet%22%20class%3D%22kagi-keycap-icon%22%3E%20%3Cstyle%3E%20%3Aroot%20%7B%20color-scheme%3A%20light%20dark%3B%20%7D%20.kagi-keycap-icon%20%7B%20color%3A%20%23000000%3B%20%7D%20%40media%20(prefers-color-scheme%3A%20dark)%20%7B%20.kagi-keycap-icon%20%7B%20color%3A%20%23FFFFFF%3B%20%7D%20%7D%20%3C%2Fstyle%3E%20%3Cpath%20d%3D%22M52%202H12C6.478%202%202%206.477%202%2011.999V52c0%205.522%204.478%2010%2010%2010h40c5.522%200%2010-4.478%2010-10V11.999C62%206.477%2057.522%202%2052%202zm5%2043.666A8.333%208.333%200%200%201%2048.667%2054H15.333A8.333%208.333%200%200%201%207%2045.666V12.333A8.332%208.332%200%200%201%2015.333%204h33.334A8.332%208.332%200%200%201%2057%2012.333v33.333z%22%20fill%3D%22currentColor%22%20fill-rule%3D%22evenodd%22%20clip-rule%3D%22evenodd%22%3E%3C%2Fpath%3E%20%3Cpath%20d%3D%22M35.262%2038.36H28.51c-1.855%200-2.138-1.99-1.792-2.789.165-.379.539-.927.892-1.376a9.06%209.06%200%200%200%204.389%201.13%209.13%209.13%200%200%200%209.12-9.122%209.1%209.1%200%200%200-3.022-6.775l.21-.212a2.32%202.32%200%200%201%201.9-.68l1.097.116v-4.47h-1.918a5.21%205.21%200%200%200-4.833%203.267%209.1%209.1%200%200%200-2.559-.366%209.13%209.13%200%200%200-6.446%202.674%209.13%209.13%200%200%200-2.675%206.446%209.08%209.08%200%200%200%201.774%205.402l-.54.416a4%204%200%200%200-.267.229c-1.848%201.723-2.695%203.98-2.211%206.49.265%201.376%201.433%202.804%202.621%203.561a5.35%205.35%200%200%200%202.823.82l7.569-.306a2.49%202.49%200%200%201%202.109%201.155l.989%201.785%204.692-1.568-.803-1.748a7.01%207.01%200%200%200-6.367-4.079m-3.263-16.519a4.367%204.367%200%200%201%204.36%204.36%204.367%204.367%200%200%201-4.36%204.359%204.367%204.367%200%200%201-4.36-4.36A4.367%204.367%200%200%201%2032%2021.842%22%20fill%3D%22%23FFB319%22%3E%3C%2Fpath%3E%20%3C%2Fsvg%3E
...
(() => {
  ...
  (function() {
    ...
    function clickAssistantModelChooser() {
      const promptBox = getActivePromptBox();
      if (promptBox) {
        const buttons = safeQuerySelectorAllLocal(promptBox, "button").filter(isElementVisibleLocal).filter((button2) => !isElementDisabled(button2)).filter((button2) => !button2.classList?.contains("icon-btn")).filter((button2) => !button2.classList?.contains("part"));
    ...
    function shortcutMatchesAssistantDefault(shortcut, managedKey) {
      ...
      if (managedKey === "voiceInput") {
        const isDefaultTarget = actionType === "custom" && customAction === "kagivoiceinput" || actionType === "selector" && selector === normalizeKagiToken('button[aria-label="Voice input"]');
        return isDefaultTarget;
      }
      if (managedKey === "lensSelect") {
        return actionType === "custom" && customAction === "kagiopenlensselect" || actionType === "selector" && selector === normalizeKagiToken('button[id="lens-select"]');
      }
      return false;
    }
    function shortcutMatchesTabbitNativeDefault(shortcut, managedKey) {
      ...
      if (managedKey === "newThread") {
        ...
        return actionType === "url" && normalizeKagiUrl(shortcut.url) === assistantUrl || actionType === "simulate" && ["ctrl+k", "cmd+k"].includes(simulateKeys);
      }
      ...
      if (managedKey === "uploadFiles") {
        return actionType === "custom" && customAction === "kagiuploadfiles" || actionType === "selector" && [
        ...
        ].includes(selector) || actionType === "simulate" && simulateKeys === "cmd+u";
      }
      if (managedKey === "modelChooser") {
        return actionType === "custom" && customAction === "kagiopenmodelchooser" || actionType === "selector" && selector === normalizeKagiToken('button[id="profile-select"]') || actionType === "simulate" && simulateKeys === "cmd+shift+m";
      }
      return false;
    }
    function buildKagiNativeShortcutFromDefault(source, replacement) {
      const current = source && typeof source === "object" ? source : {};
      const data = current.data && typeof current.data === "object" && !Array.isArray(current.data) ? { ...current.data } : replacement.data || {};
      const next = {
        ...current,
    ...
    function migrateKagiStoredShortcuts() {
      ...
      const next = stored.map((shortcut) => {
        ...
        return {
          ...
          data: source.data && typeof source.data === "object" && !Array.isArray(source.data) ? { ...source.data } : replacement.data || {}
        };
      });
    ...
    function migrateKagiTabbitNativeShortcuts() {
      ...
      const next = stored.map((shortcut) => {
        const managedKey = getKagiManagedShortcutKey(shortcut);
        const replacement = DEFAULT_SHORTCUTS_BY_KEY[managedKey];
        if (!KAGI_TABBIT_NATIVE_MIGRATION_KEYS.includes(managedKey) || !replacement || !shortcutMatchesTabbitNativeDefault(shortcut, managedKey)) {
          return shortcut;
        }
        changed = true;
      ...
      for (const managedKey of KAGI_TABBIT_NATIVE_ADDED_KEYS) {
        ...
        next.push({
          ...replacement,
          data: replacement.data && typeof replacement.data === "object" && !Array.isArray(replacement.data) ? { ...replacement.data } : {}
        });
        changed = true;
      }
      if (changed) gmSetValueLocal(STORAGE_KEYS.shortcuts, next);
    ...
    const engine = ShortcutTemplate.createShortcutEngine({
      ...
      shouldBypassIconCache: (url) => {
        return url && (url.startsWith("https://kagi.com/") || url.startsWith("https://assistant.kagi.com/") || url.startsWith("https://translate.kagi.com/"));
      },
