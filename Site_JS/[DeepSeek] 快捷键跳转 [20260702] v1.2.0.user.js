...
// @name:en        [DeepSeek] Shortcut Jump [20260702] v1.2.0
...
// @description:en Adds custom shortcuts for DeepSeek Chat with URL jumps, clicks, simulated keys, current-chat deletion, a visual settings panel, filters, dark mode, responsive layout, icon cache, and shortcut capture.

// @version        [20260702] v1.2.0
// @update-log     1.2.0: 新增删除当前聊天动作，默认快捷键 Ctrl+Backspace，并对既有配置进行一次性追加迁移。
// @update-log:en  1.2.0: Added a delete-current-chat action with Ctrl+Backspace by default and a one-time migration for existing shortcut configs.
  ...
  (function() {
    ...
    function getElementLabelText(element) {
      ...
      const parts = [
        ...
        element.id ? Array.from(document.querySelectorAll("label")).filter((label) => label.getAttribute?.("for") === element.id).map((label) => label.textContent || "").join(" ") : "",
    ...
    function getDeepSeekAttrReferenceText(element, attr) {
      try {
        return String(element?.getAttribute?.(attr) || "").split(/\s+/).map((id) => id && document.getElementById(id)).filter(Boolean).map((node) => node.innerText || node.textContent || "").join(" ");
      } catch {
        return "";
      }
    }
    ...
    function deepSeekDeleteLabelMatches(value, labels, { exact = false } = {}) {
      ...
      return (labels || []).some((label) => {
        ...
        if (exact) {
          return textValue === textLabel || compactValue === compactLabel || isRepeatedDeepSeekCompactToken(compactValue, compactLabel);
        }
    ...
    function getDeepSeekTitleToken(value) {
      const raw = String(value || "").replace(/\s*[-|–]\s*DeepSeek.*$/i, "").replace(/\s*-\s*深度求索.*$/i, "");
    ...
    function deepSeekChatLinks(root = document) {
      return queryDeepSeekAll("a[href*='/chat/s/'],a[href*='/a/chat/s/']", root).filter((link) => isVisibleElement(link) && getDeepSeekRect(link));
    }
    ...
    function findDeepSeekSidebarToggle() {
      ...
      const candidates = visibleDeepSeekSelectorElements("button,[role='button']").map((element) => ({ element, rect: getDeepSeekRect(element), text: getDeepSeekDeleteText(element), svg: getDeepSeekSvgSignature(element) })).filter((item) => item.rect && item.rect.top <= 90 && item.rect.left <= 130 && item.rect.width >= 20 && item.rect.width <= 64 && item.rect.height >= 20 && item.rect.height <= 64).filter((item) => /sidebar|menu|panel|sider|侧边栏|菜单/i.test(item.text) || /sidebar|panel|menu|m9\.67269/i.test(item.svg));
    ...
    async function clickDeepSeekSidebarToggleByPoint() {
      ...
      for (const point of points) {
        ...
        if (!dispatchDeepSeekPointerActivation(button, point) && !nativeDeepSeekClick(button) && !activateDeepSeekElement(button, void 0, { allowHidden: true })) continue;
    ...
    function deepSeekTopicMoreButton(row) {
      ...
      const add = (button, source = "", extraScore = 0) => {
        ...
        candidates.push({
          element: target,
          score: extraScore + (hasDeepSeekLayoutBox(target) ? 140 : 40) + (!compact ? 180 : 0) + (/more|options|menu|更多|菜单|选项/.test(compact) ? 220 : 0) + (/ellipsis|more|dots|circle/.test(signature) ? 120 : 0) + Math.max(0, 80 - Math.abs((rect.left + rect.right) / 2 - (rowRect.right - 28))),
          right: rect.right,
          source
        });
      };
    ...
    function deepSeekMenuRootsWithDelete(labels) {
      const roots = visibleDeepSeekSelectorElements(DEEPSEEK_DELETE_MENU_ROOT_SELECTORS).filter((root) => {
        const rect = getDeepSeekRect(root);
        const area = rect ? rect.width * rect.height : 0;
        if (area > 45e4) return false;
        const value = getDeepSeekDeleteText(root);
    ...
    async function deleteCurrentDeepSeekChat({ shortcut = null, engine: engine2 = null } = {}) {
      ...
      if (!await openDeepSeekTriggerAndClickDelete(moreButton, labels, { timeoutMs: 3e3, allowHiddenTrigger: true })) {
    ...
    function migrateDeepSeekShortcuts() {
      ...
      const next = stored.map((shortcut) => {
        ...
        return {
          ...
          iconAdaptive: shouldUseAdaptiveIcon ? true : typeof source.iconAdaptive === "boolean" ? source.iconAdaptive : replacement.iconAdaptive,
    ...
    function isDeepSeekDeleteCurrentChatShortcut(shortcut) {
      ...
      const name = normalizeDeepSeekToken(shortcut?.name);
      return name === normalizeDeepSeekToken("删除当前聊天") || name === normalizeDeepSeekToken("删除聊天") || name === normalizeDeepSeekToken("Delete Current Chat") || name === normalizeDeepSeekToken("Delete Chat");
    }
    ...
    function findDeepSeekModeOption(labels) {
      const candidates = Array.from(document.querySelectorAll("[role='radio'], input[type='radio'], [aria-checked], [aria-selected], label, button, [role='button']")).filter(isVisibleElement).map((element) => {
    ...
    const engine = ShortcutTemplate.createShortcutEngine({
      ...
      customActions: {
        deleteCurrentChat: ({ shortcut, engine: engine2 }) => deleteCurrentDeepSeekChat({ shortcut, engine: engine2 })
      },
      actionHandlers: {
