...
// @update-log     1.0.0: 修复 Delete 快捷键打开删除确认框后 Enter 可能失效的问题；确认弹窗会重新定位最新 Delete 按钮，并在点击失败时允许再次按 Enter。
// @update-log:en  1.0.0: Fixed Delete shortcut confirmation where Enter could stop responding after opening Gemini's delete dialog; the bridge re-resolves the latest Delete button and allows another Enter after a failed click.
...
(() => {
  ...
  (function() {
    ...
    async function clickDeleteConfirmButtonWithRetry({ fallbackBtn = null, engine: engine2 = null } = {}) {
    ...
    function armDeleteConfirmEnterBridge({ dialog, confirmBtn }, { engine: engine2 = null, timeoutMs = 3e4, closeCheckIntervalMs = 250 } = {}) {
      ...
      const onKeydown = (event) => {
        ...
        handlingEnter = true;
        void (async () => {
          const clicked = await clickDeleteConfirmButtonWithRetry({ fallbackBtn: candidate, engine: engine2 });
          if (cleaned) return;
          if (!clicked) {
            handlingEnter = false;
            const retryTarget2 = getDeleteConfirmTarget(candidate);
            if (isUsableDeleteConfirmButton(retryTarget2?.confirmBtn)) focusDeleteConfirmButton(retryTarget2.confirmBtn);
            return;
          }
