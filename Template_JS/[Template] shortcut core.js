// ==UserScript==
// @name           [Template] 快捷键跳转 [20260819] v1.1.3
// @name:en        [Template] Shortcut Core [20260819] v1.1.3
// @namespace      https://github.com/0-V-linuxdo/Template_shortcuts.js
// @version        [20260819] v1.1.3
// @update-log     1.1.3: SPA 跳转优先点击站内同域链接；Next.js 下克隆现有 history state 再 popstate，避免 {url} 伪造状态导致整页刷新。
// @update-log:en  1.1.3: SPA navigation clicks in-page same-origin links first; on Next.js it clones the current history state before popstate so a foreign {url} payload cannot hard-reload.
      ...
      async function prepareComposerForRun(composerEl) {
        ...
        if (!promptCleared) {
          return {
            ...
            cancelled: false,
            composer: composerRef,
            message: labels.messages?.clearPromptFailed || DEFAULT_LABELS.messages.clearPromptFailed
          };
        }
      ...
      for (let i = 0; i < cfg.loopCount; i++) {
        ...
        if (cfg.clearBeforeRun) {
          ...
          if (!prepareResult?.ok) {
            ...
            appendLoopLog(
              String(
                prepareResult?.message || labels.messages?.clearPromptFailed || DEFAULT_LABELS.messages.clearPromptFailed || labels.messages?.clearAttachmentsFailed || DEFAULT_LABELS.messages.clearAttachmentsFailed
              ),
              { level: "error" }
            );
            break;
