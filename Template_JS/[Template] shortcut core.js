 ...
 *    • ⚠️ 业务源码位于 `src/modules/`，userscript 入口位于 `src/userscript/entry.js`，头部元数据位于 `src/userscript/header.txt`。
 *    • ⚠️ 修改后运行 `npm run build`，构建会同步更新 `dist/` 与 `Template_JS/[Template] shortcut core.js`。
 *    • ⚠️ This is a generated artifact. Do not edit this file directly. Update source modules under `src/modules/`, the userscript entry at `src/userscript/entry.js`, or header metadata at `src/userscript/header.txt`, then run `npm run build` to refresh both `dist/` and `Template_JS/[Template] shortcut core.js`.
 *
 * ----------------------------------------------------------------------
 *
