...
async function startSite(runtime = {}) {
  ...
  const engine = ShortcutTemplate.createShortcutEngine({
    ...
    resolveUrlTemplate: (targetUrl, { getCurrentSearchTerm, placeholderToken }) => {
      ...
      try {
        currentKeyword = typeof getCurrentSearchTerm === "function" ? getCurrentSearchTerm() : null;
      } catch (err) {
        console.warn("[Perplexity Shortcut Script] resolveUrlTemplate error", err);
      }
      if (currentKeyword !== null && currentKeyword !== void 0) {
    ...
    text: {
      ...
      menuLabelFallback: "Perplexity - 设置快捷键"
    }
  });
  engine.init();
}
