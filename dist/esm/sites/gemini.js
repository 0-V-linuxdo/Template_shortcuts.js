...
async function startSite(runtime = {}) {
  ...
  function getKeepSidebarVisibleSetting() {
    ...
    const fn = getUserscriptApi("GM_getValue");
    if (typeof fn !== "function") return localFallback;
    try {
      const value = fn(SIDEBAR_VISIBILITY_STORAGE_KEY, DEFAULT_KEEP_SIDEBAR_VISIBLE);
  ...
  function createGeminiQuickInputAdapter({ idPrefix = "gemini" } = {}) {
    ...
    function resolveGeminiComposerElement(composerEl, { requireVisible = true } = {}) {
      const isEditableCandidate = (el) => {
        ...
        return !requireVisible || isElementVisible2(el);
      };
    ...
    function tryPasteTextIntoGeminiComposer(composerEl, text) {
      ...
      try {
        TemplateUtils?.events?.simulateClick?.(composerEl, { nativeFallback: true });
      } catch {
      }
  ...
  function handleMenuCommand(engine2, commandKey) {
    return handleMenuCommandWithId(engine2, commandKey, "");
  }
