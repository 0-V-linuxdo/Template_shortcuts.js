...
async function startSite(runtime = {}) {
  ...
  const menuCommandEventName = typeof menuBridge?.eventName === "string" && menuBridge.eventName.trim() ? menuBridge.eventName.trim() : "";
  ...
  function findMenuItemInRoot(rootEl, selector, { textMatch = null, normalize, fallbackToFirst = false } = {}) {
    ...
    if (textMatch) {
      for (const el of candidates) {
        ...
        const matched = typeof matchText === "function" ? matchText(text, textMatch, { normalize: normalizeText, element: el }) : normalizeText(text).includes(normalizeText(textMatch));
  ...
  function createGeminiQuickInputAdapter({ idPrefix = "gemini" } = {}) {
    ...
    function findGeminiDropzone(composerEl) {
      ...
      let zones = [];
      try {
        zones = Array.from(document.querySelectorAll("[xapfileselectordropzone]"));
      } catch {
        zones = [];
      }
    ...
    async function waitForGeminiAttachmentCount(containerEl, targetCount, { timeoutMs = 6e3, intervalMs = 120, shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
    ...
    async function trimGeminiUnexpectedAttachments(containerEl, expectedCount, { shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
    ...
    async function clearGeminiAttachments(composerEl, { shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
      const composer = composerEl || await focusComposer({ timeoutMs: 4e3, intervalMs: 120, shouldCancel: cancelFn, shouldIgnore: isInsideQuickInputOverlay, runtime: runtime2 });
      if (!composer) {
        return {
          ok: false,
      ...
      const clearResult = await trimGeminiUnexpectedAttachments(container, 0, { shouldCancel: cancelFn, runtime: runtime2 });
    ...
    async function waitForGeminiAttachmentChange(containerEl, prevSnapshot, { timeoutMs = 9e3, intervalMs = 120, shouldCancel = null, runtime: runtime2 = null, composerEl = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
    ...
    async function attachImageToComposer(file, composerEl, { onDiagnostics, shouldCancel = null, runtime: runtime2 = null } = {}) {
      ...
      const composer = composerEl || await focusComposer({ timeoutMs: 4e3, shouldCancel: cancelFn, shouldIgnore: isInsideQuickInputOverlay, runtime: runtime2 });
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        if (attempt > 0) {
          const retryWaitOk = await runtimeSleep(runtime2, 180, { shouldCancel: cancelFn });
          if (!retryWaitOk) {
            diagnostics.finalSnapshot = prev;
            if (typeof onDiagnostics === "function") {
              try {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        ...
        const dropWaitOk = await runtimeSleep(runtime2, 220, { shouldCancel: cancelFn });
        if (!dropWaitOk) {
          diagnostics.finalSnapshot = prev;
          if (typeof onDiagnostics === "function") {
            try {
        ...
        const dropFocusSettleOk = await runtimeSleep(runtime2, 30, { shouldCancel: cancelFn });
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        ...
        const fileInputWaitOk = await runtimeSleep(runtime2, 220, { shouldCancel: cancelFn });
        if (!fileInputWaitOk) {
          diagnostics.finalSnapshot = prev;
          if (typeof onDiagnostics === "function") {
            try {
        ...
        const fileInputFocusSettleOk = await runtimeSleep(runtime2, 30, { shouldCancel: cancelFn });
    ...
    async function waitForGeminiReadyToSend(composerEl, { requireImage = false, minAttachments = 0, timeoutMs = 45e3, intervalMs = 160, settleMs = 600, shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
      const composer = composerEl || await focusComposer({ timeoutMs: 4e3, intervalMs: 120, shouldCancel: cancelFn, shouldIgnore: isInsideQuickInputOverlay, runtime: runtime2 });
  ...
  function handleMenuCommand(engine2, commandKey) {
  ...
  function bindMenuBridgeCommands(engine2) {
    if (!menuBridge || !menuCommandEventName) return;
    const target = typeof document !== "undefined" && document && typeof document.addEventListener === "function" ? document : typeof window !== "undefined" && window && typeof window.addEventListener === "function" ? window : null;
    if (!target) return;
    target.addEventListener(menuCommandEventName, (event) => {
      const detail = event?.detail;
      if (!detail || detail.siteId !== runtime?.siteId) return;
      handleMenuCommand(engine2, detail.commandKey);
    });
  }
