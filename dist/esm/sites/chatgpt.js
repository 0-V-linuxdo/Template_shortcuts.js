...
async function startSite(runtime = {}) {
  ...
  function createChatGPTQuickInputAdapter({ idPrefix = "chatgpt" } = {}) {
    ...
    function getRuntimeNow(runtime2 = null) {
      if (runtime2 && typeof runtime2.now === "function") {
        try {
          return Number(runtime2.now()) || Date.now();
        } catch {
        }
      }
      return Date.now();
    }
    async function runtimeWaitIfPaused(runtime2 = null, { shouldCancel = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
      if (cancelFn && cancelFn()) return false;
      if (runtime2 && typeof runtime2.waitIfPaused === "function") {
        try {
          const result = await runtime2.waitIfPaused();
    ...
    async function runtimeSleep(runtime2 = null, ms = 0, { shouldCancel = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
      if (!await runtimeWaitIfPaused(runtime2, { shouldCancel: cancelFn })) return false;
      if (runtime2 && typeof runtime2.sleep === "function") {
        try {
          const result = await runtime2.sleep(ms);
      ...
      await sleep(ms);
      return runtimeWaitIfPaused(runtime2, { shouldCancel: cancelFn });
    }
    ...
    async function triggerNewChatIfStreaming({ shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
      if (!isChatGPTStreaming()) {
        return false;
      }
      await triggerNativeNewChat({ shouldCancel: cancelFn });
      const deadline = getRuntimeNow(runtime2) + 5e3;
      while (getRuntimeNow(runtime2) < deadline) {
        if (cancelFn && cancelFn()) return true;
        const waitOk = await runtimeSleep(runtime2, 200, { shouldCancel: cancelFn });
        ...
        if (!isChatGPTStreaming()) {
          ...
          await runtimeSleep(runtime2, 300, { shouldCancel: cancelFn });
          return true;
        }
      }
      return true;
    }
    async function focusComposer({ timeoutMs = 2500, intervalMs = 120, shouldCancel = null, checkStreaming = false, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
      if (checkStreaming) {
        await triggerNewChatIfStreaming({ shouldCancel: cancelFn, runtime: runtime2 });
        if (cancelFn && cancelFn()) return null;
      }
      const deadline = getRuntimeNow(runtime2) + Math.max(0, Number(timeoutMs) || 0);
      let composer = findChatGPTComposerElement();
      while (!composer && getRuntimeNow(runtime2) < deadline) {
        if (cancelFn && cancelFn()) return null;
        const waitOk = await runtimeSleep(runtime2, intervalMs, { shouldCancel: cancelFn });
      ...
      const settleOk = await runtimeSleep(runtime2, 20, { shouldCancel: cancelFn });
    ...
    async function waitForChatGPTAttachmentChange(containerEl, prevSnapshot, { timeoutMs = 9e3, intervalMs = 120, shouldCancel = null, runtime: runtime2 = null, composerEl = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
    ...
    async function waitForChatGPTAttachmentCount(containerEl, targetCount, { timeoutMs = 6e3, intervalMs = 120, shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
    ...
    async function trimChatGPTUnexpectedAttachments(containerEl, expectedCount, { shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
    ...
    async function finalizeChatGPTSingleAttachmentInsert(containerEl, prevSnapshot, { shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
      ...
      return trimChatGPTUnexpectedAttachments(containerEl, expectedCount, { shouldCancel: cancelFn, runtime: runtime2 });
    }
    async function clearChatGPTAttachments(composerEl, { shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
      let composer = resolveChatGPTComposerElement(composerEl, { requireVisible: false });
      if (!composer) {
        composer = await focusComposer({ timeoutMs: 4e3, intervalMs: 120, shouldCancel: cancelFn, runtime: runtime2 });
      }
      if (!composer) {
        return {
          ok: false,
      ...
      const clearResult = await trimChatGPTUnexpectedAttachments(container, 0, { shouldCancel: cancelFn, runtime: runtime2 });
    ...
    async function runChatGPTAttachmentInsertStrategy({
      ...
      if (beforeFocusDelayMs > 0) {
        const preFocusWaitOk = await runtimeSleep(runtime2, beforeFocusDelayMs, { shouldCancel: cancelFn });
        if (!preFocusWaitOk) {
          diagnostics.finalSnapshot = prevSnapshot;
          reportChatGPTAttachmentDiagnostics(onDiagnostics, diagnostics, { cancelled: true });
          return { ok: false, cancelled: true, snapshot: prevSnapshot };
      ...
      const focusSettleOk = await runtimeSleep(runtime2, 30, { shouldCancel: cancelFn });
      if (!focusSettleOk) {
        diagnostics.finalSnapshot = prevSnapshot;
        reportChatGPTAttachmentDiagnostics(onDiagnostics, diagnostics, { cancelled: true });
        return { ok: false, cancelled: true, snapshot: prevSnapshot };
      ...
      const normalized = await finalizeChatGPTSingleAttachmentInsert(container, prevSnapshot, { shouldCancel: cancelFn, runtime: runtime2 });
    ...
    async function attachImageToComposer(file, composerEl, { onDiagnostics, shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
      const urlGuard = getChatGPTUrlGuardResult();
      ...
      if (!composer) {
        composer = await focusComposer({ timeoutMs: 4e3, shouldCancel: cancelFn, runtime: runtime2 });
      }
    ...
    async function attachImagesToComposer(files, composerEl, { onDiagnostics, shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
      const urlGuard = getChatGPTUrlGuardResult();
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        const betweenFilesWaitOk = await runtimeSleep(runtime2, 120, { shouldCancel: cancelFn });
    ...
    async function waitForChatGPTNewChatReady({ timeoutMs = 12e3, intervalMs = 160, settleMs = 300, shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
      const deadline = getRuntimeNow(runtime2) + Math.max(0, Number(timeoutMs) || 0);
      ...
      while (getRuntimeNow(runtime2) < deadline) {
        ...
        if (isChatGPTRootUrl()) {
          if (!stableSince) stableSince = getRuntimeNow(runtime2);
          if (getRuntimeNow(runtime2) - stableSince >= settle) {
    ...
    async function waitForChatGPTReadyToSend(composerEl, { requireImage = false, minAttachments = 0, timeoutMs = 45e3, intervalMs = 160, settleMs = 600, shouldCancel = null, runtime: runtime2 = null } = {}) {
      const cancelFn = typeof shouldCancel === "function" ? shouldCancel : null;
      let composer = resolveChatGPTComposerElement(composerEl);
      if (!composer) {
        composer = await focusComposer({ timeoutMs: 4e3, intervalMs: 120, shouldCancel: cancelFn, runtime: runtime2 });
  ...
  if (bootstrapMenuManaged && typeof window !== "undefined" && window && typeof window.addEventListener === "function") {
    ...
    if (menuBridge?.managedByBootstrap && typeof menuBridge.setCommandHandler === "function") {
      const clearQuickInputHandler = () => {
        try {
          menuBridge.setCommandHandler(MENU_COMMAND_KEYS.quickInput, null);
        } catch {
        }
      };
