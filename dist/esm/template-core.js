...
function getDomConstructor(name) {
  const scope = getGlobalScope();
...
function hasUserscriptApi(name) {
...
function sanitizeFragment(fragment) {
  ...
  try {
    ...
    const walker = NodeFilterCtor ? createTreeWalker(fragment, NodeFilterCtor.SHOW_ELEMENT, null) : null;
    if (!walker) return;
...
function normalizeImageFiles(value) {
  ...
  return Array.from(value || []).filter((file) => file && file instanceof FileCtor && String(file.type || "").startsWith("image/")).map((file, index) => {
...
var index_default = ShortcutTemplate;
export {
  URL_METHODS,
  VERSION,
  createShortcutEngine,
