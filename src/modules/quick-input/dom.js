...
function clampInt(value, { min = 0, max = 9999, fallback = 0 } = {}) {
        ...
        function clearContentEditableDomValue(el) {
            ...
            if (!cleared && selectContentEditableText(el)) {
                try { globalThis.document?.execCommand?.("delete", false, null); } catch {}
                cleared = isEditableValueMatch(el, "");
            }

            if (!cleared && selectContentEditableText(el)) {
                try { globalThis.document?.execCommand?.("insertText", false, ""); } catch {}
                cleared = isEditableValueMatch(el, "");
            }
        ...
        function insertContentEditableTextInChunks(el, text, { maxChunkSize = 1024 } = {}) {
            ...
            for (let index = 0; index < chunks.length; index++) {
                ...
                try { globalThis.document?.execCommand?.("insertText", false, chunks[index]); } catch {}
            }
        ...
        function setInputValue(el, value) {
            ...
            if (el.isContentEditable) {
                ...
                if (selectContentEditableText(el)) {
                    try { globalThis.document?.execCommand?.("insertText", false, text); } catch {}
                    inserted = isEditableValueMatch(el, text);
                }

                if (!inserted) {
                    clearContentEditableDomValue(el);
                    // 长文本在部分站点会被单次 insertText 吞掉，分块插入更稳。
