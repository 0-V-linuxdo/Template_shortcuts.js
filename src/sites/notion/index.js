...
(function() {
    ...
    async function selectSearchScopeAction({ engine } = {}) {
        const trigger = findSettingsTriggerElement();
        if (!trigger) return false;
        syncNotionShortcutIconFromElement(engine, "selectSearchScope", trigger);
        const root = await ensureSettingsMenuOpen(trigger);
        if (!root) return false;
        const row = findWebAccessMenuItem(root);
        if (row) syncNotionWebAccessShortcutIconFromElement(engine, row);
        return true;
    }

    async function toggleResearchModeAction({ engine } = {}) {
        const directTrigger = findResearchModeTriggerElement();
        if (directTrigger) {
            syncNotionShortcutIconFromElement(engine, "toggleResearchMode", directTrigger);
        ...
        let researchItem = findSettingsMenuItemByText(root, textLooksLikeResearchModeMenuItem) ||
            findOpenSettingsMenuItemByText(textLooksLikeResearchModeMenuItem);
        if (researchItem) {
            syncNotionShortcutIconFromElement(engine, "toggleResearchMode", researchItem);
            const target = getClickableActionElement(researchItem, root) || researchItem;
            if (!simulateClickElement(target, { nativeFallback: true })) return false;
