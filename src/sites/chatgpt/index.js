...
(function() {
    ...
    function createChatGPTQuickInputAdapter({ idPrefix = "chatgpt", engine = null } = {}) {
        ...
        function findChatGPTComposerTextNode(root, { backwards = false } = {}) {
            ...
            const walker = document.createTreeWalker(
                ...
                {
                    acceptNode(node) {
                        return String(node?.nodeValue || "").length > 0
                            ? textFilter.FILTER_ACCEPT
                            : textFilter.FILTER_SKIP;
                    }
                }
            );
        ...
        function getChatGPTComposerBoundaryPoint(composerEl, { position = "end" } = {}) {
            ...
            let blocks = [];
            try { blocks = Array.from(composer.querySelectorAll(CHATGPT_TEXT_BLOCK_SELECTOR)); } catch { blocks = []; }
            ...
            let brs = [];
            try { brs = Array.from(target.querySelectorAll?.("br") || []); } catch { brs = []; }
        ...
        function findChatGPTFallbackTextarea(composerEl) {
            ...
            try { pushScope(composer?.closest?.("[class*='prosemirror-parent']") || null); } catch { }
        ...
        function selectAllChatGPTComposerContent(composerEl) {
            ...
            try {
                ...
                if (!selection) return false;
                const range = document.createRange();
                // Select the complete editor tree so top-level ProseMirror atom nodes are
                // cleared together with normal text blocks.
        ...
        async function tryInsertTextIntoChatGPTComposerViaExecCommand(composerEl, text) {
            ...
            for (const chunk of chunks) {
                ...
                try { liveComposer.focus?.(); } catch { }
                try { TemplateUtils?.events?.simulateClick?.(liveComposer, { nativeFallback: true }); } catch { }
        ...
        async function tryBridgeTextViaChatGPTFallbackTextarea(composerEl, text) {
            ...
            try {
                ...
                try { textarea.dispatchEvent(new Event("input", { bubbles: true, cancelable: false })); } catch { }
            }

            try { textarea.dispatchEvent(new Event("change", { bubbles: true })); } catch { }
            return true;
        }
        ...
        function getChatGPTAttachmentSnapshot(containerEl) {
            ...
            try {
                const headerItems = Array.from(scope.querySelectorAll([
                    ...
                    .filter(card => isChatGPTAttachmentCard(card));
            ...
            if (cards.size === 0) {
                try {
                    ...
                    for (const el of sizeContainers) {
                        ...
                        if (!isElementVisible(el)) continue;
                        if (
                            el.closest?.("[class*='grid-area:header'], [class*='grid-area\\:header']")
                            || el.closest?.(".group.text-token-text-primary.relative.inline-block")
                            || el.closest?.("[role='group'][aria-label][class*='group/file-tile']")
                            || el.closest?.("[role='group'][aria-label]")
                        ) {
                            fileCount++;
                        }
                    }
            ...
            for (let i = 0; i < list.length; i++) {
                ...
                if (!result?.ok) {
                    const detail = typeof result?.message === "string" && result.message.trim()
                        ? result.message.trim()
                        : qiText("attachFailed", {}, "Image paste failed: no image preview was detected in the input box.");
    ...
    function ensureQuickInputController(engine) {
        ...
        const adapter = createChatGPTQuickInputAdapter({ idPrefix: "chatgpt", engine });
        if (!adapter) {
    ...
    const normalizeAspectRatioText = typeof chatgptDomUtils.normalizeText === "function"
        ? chatgptDomUtils.normalizeText
        : normalizeChatgptUiText;
    const safeQueryAll = typeof chatgptDomUtils.safeQuerySelectorAll === "function"
        ? chatgptDomUtils.safeQuerySelectorAll
        : (root, selector) => {
    ...
    const isVisibleElement = typeof chatgptDomUtils.isVisible === "function"
        ? chatgptDomUtils.isVisible
        : (element) => {
    ...
    const simulateClickElement = typeof chatgptEventUtils.simulateClick === "function"
        ? chatgptEventUtils.simulateClick
        : (element) => {
            try {
                element?.click?.();
                return true;
    ...
    const escapeAttributeSelector = typeof chatgptDomUtils.escapeForAttributeSelector === "function"
        ? chatgptDomUtils.escapeForAttributeSelector
        : (value) => String(value || "").replace(/\\/g, "\\\\").replace(/"/g, '\\"');
    const sleepMenuStep = typeof TemplateUtils?.sleep === "function"
        ? TemplateUtils.sleep
    ...
    function isChatgptUsableActionElement(element) {
        ...
        try { if (element.disabled) return false; } catch {}
    ...
    function scoreChatgptToolbarMenuRootCandidate(menuElement) {
        ...
        let score = hits * 1000 + Math.min(itemCount, 12) * 40;
    ...
    function findChatgptToolbarMenuRoot(triggerEl = null) {
        if (triggerEl) {
            ...
            if (triggerId) {
                const labelledMenu = safeQueryAll(document, SELECTORS.popupMenuRoot).find((menuElement) => (
                    String(menuElement.getAttribute?.("aria-labelledby") || "").trim() === triggerId &&
                    isChatgptToolbarMenuRootCandidate(menuElement)
                ));
                if (labelledMenu) return labelledMenu;
            }
        }

        const candidates = safeQueryAll(document, SELECTORS.popupMenuRoot)
            .map(element => ({ element, score: scoreChatgptToolbarMenuRootCandidate(element) }))
            .filter(item => item.score >= 0);
        if (candidates.length === 0) {
            candidates.push(
                ...safeQueryAll(document, "body div")
                    .filter(isChatgptLooseToolbarMenuRootCandidate)
                    .map(element => ({ element, score: scoreChatgptToolbarMenuRootCandidate(element) + 80 }))
                    .filter(item => item.score >= 0)
            );
        }
    ...
    function findChatgptDirectToolbarItem({
        ...
        const items = safeQueryAll(document, selector)
            .filter(item => !isChatgptMenuScopedElement(item))
            .filter(isChatgptUsableActionElement)
            .filter(isChatgptComposerAdjacentElement);
        if (items.length === 0) return null;
    ...
    function scoreAspectRatioTriggerCandidate(element) {
        ...
        const hasComposerBtnClass = (() => {
            try { return element.classList?.contains("composer-btn") || false; } catch { return false; }
        })();
    ...
    function findComposerModeMenuRoot(triggerEl = null, { textMatch = null } = {}) {
        ...
        const visibleMenus = safeQueryAll(document, SELECTORS.composerModeMenuRoot)
            .filter(menu => isComposerModeMenuCandidate(menu, { textMatch }));
        if (visibleMenus.length === 0) return null;
        visibleMenus.sort((a, b) => {
            const aBottom = Number(a.getBoundingClientRect?.().bottom || 0);
            const bBottom = Number(b.getBoundingClientRect?.().bottom || 0);
    ...
    function getChatgptMenuActionSpec(shortcut) {
        ...
        const iconIds = normalizeChatgptMenuIconIds(menu.iconIds !== undefined ? menu.iconIds : menu.iconId);
        const canonicalMatch = menuTarget
            ? createChatgptMenuTargetMatcher(menuTarget, iconIds)
            : createChatgptMenuIconMatcher(iconIds);
    ...
    function cloneChatgptShortcutItem(shortcut) {
        ...
        const data = shortcut.data && typeof shortcut.data === "object" && !Array.isArray(shortcut.data)
